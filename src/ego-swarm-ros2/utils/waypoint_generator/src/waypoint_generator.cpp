...
public:
    ...
    WaypointGenerator(std::string name) : Node(name)
    {
        ...
        auto sub3 = this->create_subscription<geometry_msgs::msg::PoseStamped>(
            ...
            std::bind(&WaypointGenerator::traj_start_trigger_callback, this, std::placeholders::_1));
...
private:
    ...
    void load_waypoints(const rclcpp::Time& time_base)
    {
        ...
        for (int i = 0; i < seg_cnt; ++i)
        {
            ...
            {
                rcpputils::assert_true(
                    rclcpp::Time(waypointSegments[i - 1].header.stamp) < rclcpp::Time(
    ...
    void goal_callback(geometry_msgs::msg::PoseStamped::ConstSharedPtr msg)
    {
        /*    if (!is_odom_ready) {
