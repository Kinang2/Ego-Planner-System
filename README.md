...
# 2. 下载 QGroundControl 地面站
mkdir -p ~/github
cd ~/github
wget https://d176tv9ibo4jno.cloudfront.net/latest/QGroundControl-x86_64.AppImage
chmod +x QGroundControl-x86_64.AppImage
# 启动方式（仿真时在后台运行即可）
# ~/github/QGroundControl-x86_64.AppImage

# 3. 下载并编译 PX4-Autopilot v1.17.0-alpha1
mkdir -p ~/github
cd ~/github
git clone https://github.com/PX4/PX4-Autopilot.git --branch v1.17.0-alpha1 --recursive
cd PX4-Autopilot

# 安装 PX4 依赖（首次运行需联网，约需几分钟）
bash ./Tools/setup/ubuntu.sh

# 编译 PX4 SITL（Gazebo Harmonic）
make px4_sitl

# 4. 确认 PX4-Autopilot 路径正确（launch 文件中默认为 ~/github/PX4-Autopilot）
